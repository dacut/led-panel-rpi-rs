...
int ioctl_ccompat_test() {
    FILE *fp = fopen("crates/gpio-linux-char/src/ioctl/ccompat_tests.rs", "w");
    if (fp == NULL) {
        perror("ioctl-test.rsinc");
        return 1;
    }
    ...
    EQ_CHECK2(IOC_DIRBITS, _IOC_DIRBITS);
    EQ_CHECK2(IOC_SIZEBITS, _IOC_SIZEBITS);
    fprintf(fp, "}\n");
    fclose(fp);
}

int gpio_ioctl_ccompat_test() {
    FILE *fp = fopen("crates/gpio-linux-char/src/gpio_ioctl/ccompat_tests.rs", "w");
    if (fp == NULL) {
